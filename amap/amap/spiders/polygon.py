...
class PolygonSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for area in area_list:
            city = area['city']
            lat_min, lat_max, lng_min, lng_max = float(area["lat_min"]), float(area["lat_max"]), float(
                area["lng_min"]), float(area["lng_max"])
            ceil = getCeil(lat_min, lat_max, lng_min, lng_max)
            for c in ceil:
                start_url = self.base_url.format(c, 1)
                yield scrapy.Request(start_url, callback=self.parse, meta={'polygon': c, "name": city})
    ...
    def parse(self, response):
        ...
        if count >= 800:
            print("poi数据大于800")
            xmin, ymax = response.meta['polygon'].split('|')[0].split(',')
            xmax, ymin = response.meta['polygon'].split('|')[1].split(',')
            midY = round(((float(ymin) + float(ymax)) / 2), 6)
            params1 = str(xmin) + ',' + str(ymax) + '|' + str(xmax) + ',' + str(midY)
            params2 = str(xmin) + ',' + str(midY) + '|' + str(xmax) + ',' + str(ymin)
            url1 = self.base_url.format(params1, 1)
            url2 = self.base_url.format(params2, 1)
            yield scrapy.Request(url1, callback=self.parse, meta={'polygon': params1, "name": response.meta["name"]})
            yield scrapy.Request(url2, callback=self.parse, meta={'polygon': params2, "name": response.meta["name"]})
        elif 0 < count < 800:
            item = AmapItem()
